...
class JSONRPCProtocol(RPCBatchProtocol):
    ...
    def parse_reply(self, data):
        ...
        try:
            rep = json.loads(data,object_pairs_hook = collections.OrderedDict)
    ...
    def parse_request(self, data):
        if six.PY3 and isinstance(data, bytes):
            ...
            # end; decoding non-unicode strings back into unicode
            data = data.decode()
#         logger.info("jsonrpc parse_request:%s" %str(json.loads(data)))
        try:
            req = json.loads(data,object_pairs_hook = collections.OrderedDict)
        except Exception as e:
